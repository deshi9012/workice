				...
				eventSources: [
					{
						events: [
							...
							{{--},--}}
								{{--@endforeach--}}
								@foreach ($lead->appointments as $key => $event)
							{
								title: '{{ addslashes($event->name) }}',
								start: '{{  date('Y-m-d H:i', strtotime($event->start_time))  }}',
								end: '{{  date('Y-m-d H:i', strtotime($event->finish_time))  }}',
								url: '{{  route('calendar.view.appointment', ['id' => $event->id, 'module' => 'appointments'])  }}',
								color: '{{ $event->color }}'
							},
							@endforeach
						],
