...
class SetupController extends Controller
{
    ...
    {
        $data['categories'] = $this->category->whereModule('knowledgebase')->get();
        return view('knowledgebase::_ajax.categories')->with($data);
    }
    ...
    public function saveCategory(CategoryRequest $request)
    {
        if ($request->ajax()) {
            if ($category = $this->category->create($request->all())) {
                $category->update(['order' => $this->category->whereModule('knowledgebase')->count() + 1]);
                $html = view('knowledgebase::_ajax.newCategoryHtml', compact('category'))->render();
    ...
    public function editCategory($id = null)
    {
        ...
        return view('knowledgebase::modal.updateCategory')->with($data);
    }
